"""
Set of functions for estimating the emprical banding effect
"""
...
def estimate_empirical_banding(
    subject_dirs,
    rois=["wm"],
    tr=8,
    method="separate",
    ...
    """
    Estimates the slice-dependent banding effect on the given subject's
    ...
    for tissue in rois:
        ...
        for n1, subject_dir in enumerate(subject_dirs):
            try:
                mask_dirs = [
                    ...
                    / "ASL/Calib"
                    / c
                    / f"SEbased_t1mask{suf}/distortion_correction/masks"
        ...
        plt_name = outdir / "mean_voxel_count_sebased.png"
        plt.savefig(plt_name)
        ...
        for subject_dir in subject_dirs:
            ...
            method_dir = (
                subject_dir
                / f"ASL/calibration/calib0/SEbased_t1mask{suf}/distortion_correction"
            )
            calib_name = method_dir / "calib0_bc.nii.gz"
            ...
            eb_dir = method_dir / "empirical_banding_correction"
            eb_dir.mkdir(exist_ok=True, parents=True)
            scaling_name = eb_dir / f"eb_SFs_{method}_{tissue}_sebased.nii.gz"
            scaling_img.save(scaling_name)

            # apply scaling factors to image to perform correction
            eb_name = eb_dir / f"calib0_eb_{method}_{tissue}_sebased.nii.gz"
            eb_img = Image(calib_img.data * scaling_factors, header=calib_img.header)
            eb_img.save(str(eb_name))
    return errors

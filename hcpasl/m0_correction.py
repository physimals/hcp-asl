...
def initial_corrections_m0(
    ...
    for calib_path in (calib0, calib1):
        ...
        if gd_corr:
            logging.info(
                f"Applying gradient distortion correction to {calib_corr.stem}"
            )
        ...
        if not nobandingcorr:
            logging.info(
                f"Applying empirical banding scaling factors to {calib_corr.stem}"
            )
            mt_sfs = np.loadtxt(mt_factors)
            assert len(mt_sfs) == calib_corr.img.shape[2]
            calib_corr = calib_corr.correct_from_data(
                calib_dir / "MTCorr", "mtcorr", calib_corr.img.get_fdata() * mt_sfs
            )

        logging.info(f"Getting registration of {calib_corr.stem} to structural")
        reg_dir = calib_dir / "structural_registration"
        reg_dir.mkdir(exist_ok=True, parents=True)
        register_asl2struct(calib_corr.path, struct_name, fsdir, reg_dir)
        ...
        calib_corr = calib.correct_from_image(
            ...
            suffix,
            dc_calibspc_warp.apply_to_image(
        ...
        calib_corr = calib_corr.correct_from_data(
            biascorr_dir, "bc", calib_corr.img.get_fdata() / bias_img.get_fdata()
        )

        if not nobandingcorr:
            logging.info(
                f"Performing empirical banding correction to {calib_corr.stem}"
            )
            calib_corr = calib_corr.correct_from_data(
                biascorr_dir, "mt", calib_corr.img.get_fdata() * mt_sfs
            )
